<template>
    <div class="flex items-center justify-center min-h-screen px-4 py-12 bg-gray-50 sm:px-6 lg:px-8">
      <div class="w-full max-w-md space-y-8">
        ...
        <div v-if="apiError" class="p-3 text-sm text-red-700 bg-red-100 border border-red-400 rounded-md">
          {{ apiError }}
        </div>
  ...
  const handleSubmit = async () => {
    ...
    if (validateForm()) {
      try {
        const response = await store.dispatch('signup', { name: name.value, email: email.value, password: password.value })
        if (response) {
          router.push('/')
        } else {
